...
def set_rtc(dt):
    ...
    bus.write_i2c_block_data(RTC_ADDRESS, 0x04, [second, minute, hour, date, weekday, month, year])
...
def play_item(item_id, item_type, delay, is_schedule=False):
    with play_lock:
        if pygame.mixer.music.get_busy():
            logging.info(f'Skippe Wiedergabe für {item_type} {item_id}, da andere läuft')
            return
        set_sink(DAC_SINK)
        activate_amplifier()
        time.sleep(delay)
        ...
        try:
            if item_type == 'file':
                cursor.execute("SELECT filename FROM audio_files WHERE id=?", (item_id,))
                ...
                cursor.execute("SELECT f.filename FROM playlist_files pf JOIN audio_files f ON pf.file_id = f.id WHERE pf.playlist_id=?", (item_id,))
...
def is_bt_audio_active():
    ...
    for sink in sinks:
        ...
        inputs = subprocess.getoutput(f"pactl list short sink-inputs | grep {sink_name}")
        if inputs.strip():
            return True
    return False
...
def index():
    ...
    current_volume = subprocess.getoutput('pactl get-sink-volume @DEFAULT_SINK@ | grep -oP "\\d+%" | head -1') or 'Unbekannt'
    status = {
    ...
    return render_template('index.html', files=files, playlists=playlists, schedules=schedules, status=status)
...
def delete(file_id):
    ...
    cursor.execute("DELETE FROM schedules WHERE item_id=? AND item_type='file'", (file_id,))
    conn.commit()
...
def add_to_playlist():
    ...
    cursor.execute("INSERT INTO playlist_files (playlist_id, file_id) VALUES (?, ?)", (playlist_id, file_id))
    conn.commit()
...
def delete_playlist(playlist_id):
    ...
    cursor.execute("DELETE FROM schedules WHERE item_id=? AND item_type='playlist'", (playlist_id,))
    conn.commit()
...
def bluetooth_auto_accept():
    p = subprocess.Popen(['sudo', 'bluetoothctl'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
    commands = [
        'power on',
        'discoverable on',
        'pairable on',
